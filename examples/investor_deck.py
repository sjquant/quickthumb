...
def brand_bar(c: Canvas) -> Canvas:
    ...
    return (c
        .shape(shape="rectangle", position=(0, 0), width=640, height=2, color=PURPLE)
        .shape(shape="rectangle", position=(640, 0), width=640, height=2, color=CYAN)
    )


def logo(c: Canvas, x: str = "10%", y: str = "8.5%") -> Canvas:
    return c.text(
        content="VELA", font="Roboto", size=18, color=PURPLE,
        weight=700, letter_spacing=4, position=(x, y),
    )


def label(c: Canvas, text: str, x: str, y: str, *, animation=None) -> Canvas:
    return c.text(
        content=text, font="Roboto", size=19, color=MUTED,
        weight=400, letter_spacing=3, position=(x, y),
        animation=animation,
    )


def divider(c: Canvas, y: str, *, x: str = "10%", w: int = 960,
            animation=None) -> Canvas:
    return c.shape(
        shape="rectangle", position=(x, y), width=w, height=1,
        color=RULE, animation=animation,
    )
...
s1 = s1.shape(
    shape="ellipse", position=("92%", "14%"), width=560, height=560,
    color=PURPLE, opacity=0.07, align=("center", "middle"),
)
s1 = s1.shape(
    shape="ellipse", position=("4%", "90%"), width=320, height=320,
    color=CYAN, opacity=0.06, align=("center", "middle"),
)

s1 = logo(s1)
s1 = label(s1, "SERIES A  ·  INVESTOR PRESENTATION", "10%", "17.5%",
           animation=Fade(duration=0.45))

# Three-line display headline; gradient fill draws the eye across the break.
s1 = s1.text(
    content="Make better\ndecisions,\nfaster.",
    font="NotoSerif", size=108, fill=BRAND, weight=900,
    line_height=1.02, position=("10%", "27%"),
    effects=[DROP],
...
s1 = s1.text(
    ...
    font="Roboto", size=27, color=OFFWHITE, weight=300,
    line_height=1.6, position=("10%", "81%"),
    animation=Fade(duration=0.45, trigger="after_previous"),
)

s1 = s1.text(
    content="Confidential", font="Roboto", size=17, color=MUTED,
    weight=300, position=("90%", "93%"), align=("right", "top"),
)
...
s2 = s2.text(
    content="73%",
    font="NotoSerif", size=210, fill=BRAND, weight=900,
    position=("50%", "24%"), align="center",
    effects=[DROP],
...
s2 = s2.text(
    ...
    font="Roboto", size=30, color=OFFWHITE, weight=300,
    position=("50%", "73%"), align="center",
...
s2 = s2.group(
    children=[
        {"type": "text", "content": "Siloed dashboards",
         "font": "Roboto", "size": 24, "color": MUTED, "weight": 300},
        {"type": "text", "content": "·",
         "font": "Roboto", "size": 24, "color": RULE},
        {"type": "text", "content": "Alert fatigue",
         "font": "Roboto", "size": 24, "color": MUTED, "weight": 300},
        {"type": "text", "content": "·",
         "font": "Roboto", "size": 24, "color": RULE},
        {"type": "text", "content": "No clear owner",
         "font": "Roboto", "size": 24, "color": MUTED, "weight": 300},
    ],
    direction="row", gap=32,
    position=("50%", "87%"), align=("center", "top"), item_align="center",
    animation=Fade(duration=0.4, trigger="after_previous"),
)
...
s3 = s3.text(
    ...
    font="NotoSerif", size=64, fill=HERO, weight=900,
    position=("50%", "20%"), align="center",
    effects=[DROP],
    animation=Fade(duration=0.5),
)

s3 = divider(s3, "35%", x="50%", w=1040,
             animation=Wipe(direction="right", duration=0.4, trigger="after_previous"))
...
for x_pct, title, accent, _hex, body in FEATURES:
    ...
    s3 = s3.shape(
        shape="ellipse", position=(x_pct, "44%"), width=56, height=56,
        color=accent, opacity=0.15, align=("center", "middle"),
        animation=Fade(duration=0.4),
    )
    s3 = s3.shape(
        shape="ellipse", position=(x_pct, "44%"), width=24, height=24,
        color=accent, align=("center", "middle"),
        effects=[Glow(radius=16, color=accent, opacity=0.5)],
        animation=Fade(duration=0.4, trigger="with_previous"),
    )

    s3 = s3.text(
        content=title, font="Roboto", size=38, fill=grad, weight=700,
        line_height=1.1, position=(x_pct, "53%"), align="center",
        animation=Wipe(direction="up", duration=0.35, trigger="with_previous"),
    )

    s3 = s3.text(
        content=body, font="Roboto", size=22, color=OFFWHITE, weight=300,
        line_height=1.6, position=(x_pct, "70%"), align="center",
...
for x_pct, value, sublabel, accent in METRICS:
    ...
    s4 = s4.text(
        content=value, font="NotoSerif", size=92, fill=num_grad, weight=900,
        position=(x_pct, "50%"), align="center",
        effects=[DROP],
        animation=Box(direction="in", duration=0.45),
    )
    s4 = s4.text(
        content=sublabel, font="Roboto", size=25, color=MUTED, weight=400,
        letter_spacing=1, position=(x_pct, "72%"), align="center",
        animation=Fade(duration=0.3, trigger="after_previous"),
    )

s4 = divider(s4, "84%", x="50%", w=960,
...
s4 = s4.text(
    ...
    font="Roboto", size=22, color=MUTED, weight=300, italic=True,
    position=("50%", "89%"), align="center",
    animation=Fade(duration=0.4, trigger="after_previous"),
)
...
s5 = s5.shape(
    shape="ellipse", position=("88%", "18%"), width=640, height=640,
    color=WHITE, opacity=0.04, align=("center", "middle"),
)
s5 = s5.shape(
    shape="ellipse", position=("8%", "82%"), width=380, height=380,
    color=WHITE, opacity=0.03, align=("center", "middle"),
)
...
s5 = s5.text(
    ...
    font="NotoSerif", size=108, color=WHITE, weight=900,
    line_height=1.02, position=("50%", "28%"), align="center",
    effects=[DROP],
...
s5 = s5.text(
    ...
    font="Roboto", size=28, color="#DDD6FE", weight=300,
    letter_spacing=1, position=("50%", "72%"), align="center",
    animation=Fade(duration=0.5, trigger="after_previous"),
)

s5 = s5.text(
    content="hello@vela.so",
    font="Roboto", size=21, color="#A78BFA", weight=400,
    letter_spacing=1, position=("50%", "84%"), align="center",
    animation=Fade(duration=0.4, trigger="after_previous"),
)
